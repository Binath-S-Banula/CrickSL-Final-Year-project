  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', background: 'var(--bg-primary)' }}>
      ...
      <div style={{
        ...
        {STATS.map(s => (
          <div key={s.label}>
            <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: '2rem', fontWeight: 700, color: '#f59e0b' }}>
              {s.value}
            </div>
      ...
      <div style={{ padding: '4rem 2rem', maxWidth: '1300px', margin: '0 auto', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <h2 style={{
            fontFamily: 'Rajdhani, sans-serif', fontSize: '1.6rem',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '1.5rem' }}>
          {MODULES.slice(0, 3).map(m => <ModuleCard key={m.path} m={m} navigate={navigate} />)}
        </div>

        {/* Bottom row — 2 cards centered */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem', maxWidth: '900px', margin: '0 auto' }}>
          {MODULES.slice(3).map(m => <ModuleCard key={m.path} m={m} navigate={navigate} />)}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '1.25rem' }}>
          Built With
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '0.75rem' }}>
          {['FastAPI', 'PostgreSQL', 'React + Vite', 'scikit-learn', 'Random Forest', 'Cricsheet Dataset', 'Open-Meteo API', 'JWT Auth', 'GitHub Actions CI'].map(t => (
            <span key={t} style={{
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
        {m.badges.map(b => <span key={b} className={`badge ${m.badgeClass}`}>{b}</span>)}
      </div>
    </div>
  )
}
