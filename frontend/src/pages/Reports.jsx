...
function generateStrategies(prediction, report, tossDecision, tossWinner, team2) {
  ...
  if (slProb < 35) {
    ...
    cards.push({ type: 'warning', icon: '⚠️', title: 'Slight Underdog — Smart Execution Can Close the Gap',
      bullets: [
    ...
    cards.push({ type: 'positive', icon: '✅', title: 'Slight Favourite — Maintain Discipline and Avoid Complacency',
      bullets: [
    ...
    cards.push({ type: 'positive', icon: '🏆', title: 'Strong Favourite — Dominate Early and Close Out Professionally',
      bullets: [
  ...
  if (dewHigh && slBattingFirst) {
    ...
    cards.push({ type: 'positive', icon: '💧', title: 'High Dew — Chasing Conditions Favour Sri Lanka',
      bullets: [
    ...
    cards.push({ type: 'warning', icon: '💧', title: 'Moderate Dew Expected — Monitor and Adapt',
      bullets: [
  ...
  if (slBattingFirst && chasePct > 55) {
    ...
    cards.push({ type: 'warning', icon: '🏟️', title: `Venue Favours Batting First (${Math.round(batFirstPct)}% bat-first wins) — Chase Smartly`,
      bullets: [
    ...
    cards.push({ type: 'neutral', icon: '🏟️', title: 'Balanced Venue — Execution Decides the Match',
      bullets: [
  ...
  if (poorForm) {
    ...
    cards.push({ type: 'positive', icon: '📈', title: 'SL in Strong Form — Carry That Momentum In',
      bullets: [
  ...
  if (poorH2H) {
    ...
    cards.push({ type: 'positive', icon: '⚔️', title: `Strong H2H vs ${team2} — Use the Psychological Advantage`,
      bullets: [
  ...
  const runPrediction = async () => {
    ...
    try {
      const res = await api.post('/predict/', {
        venue_name: venue, toss_winner: tossWinner,
        toss_decision: tossDecision, team1: 'Sri Lanka', team2,
      })
            ...
            <>
                ...
                <div style={s.dataWarning}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: '0.2rem 0 0', fontSize: '0.82rem', color: '#94a3b8', lineHeight: 1.5 }}>
                      ...
                      <strong style={{ color: '#fbbf24' }}>{report.player_data_years} years</strong> of available data.
                    </p>
                  </div>
                </div>
              )}
              ...
              {showPredConfig && (
                <div style={s.mlSection} className="no-print">
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                    ...
                    <span style={{ background: 'rgba(59,130,246,0.15)', border: '1px solid rgba(59,130,246,0.3)', color: '#60a5fa', borderRadius: '20px', padding: '0.3rem 0.9rem', fontSize: '0.75rem', fontWeight: 600 }}>
                      Random Forest v2.0
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-end', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                      <label style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>Toss Winner</label>
                      <select style={s.predSelect} value={tossWinner} onChange={e => setTossWinner(e.target.value)}>
                        <option value="Sri Lanka">Sri Lanka</option>
                        <option value={team2}>{team2}</option>
                      </select>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                      <label style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>Toss Decision</label>
                      <select style={s.predSelect} value={tossDecision} onChange={e => setTossDecision(e.target.value)}>
                  ...
                  {prediction && !prediction.error && (
                    <div style={s.predResult} className="pred-card">
                      {/* Probability display */}
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
                        <div style={{ textAlign: 'center', flex: 1, minWidth: '120px', padding: '1rem', borderRadius: '12px',
                          background: isFavoured ? 'rgba(16,185,129,0.06)' : 'rgba(255,255,255,0.02)',
                          border: isFavoured ? '1px solid rgba(16,185,129,0.2)' : 'none' }}>
                          <div style={{ fontSize: '2.2rem', marginBottom: '0.4rem' }}>🇱🇰</div>
                          <div style={{ fontSize: '0.88rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Sri Lanka</div>
                          <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: '3rem', fontWeight: 800, lineHeight: 1, marginBottom: '0.5rem', color: isFavoured ? '#10b981' : '#94a3b8' }}>{slProb}%</div>
                          {isFavoured && <span style={{ background: 'rgba(16,185,129,0.15)', color: '#10b981', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '20px', padding: '0.2rem 0.7rem', fontSize: '0.7rem', fontWeight: 700 }}>FAVOURED ✓</span>}
                        </div>
                        <div style={{ textAlign: 'center', padding: '0 1rem' }}>
                          <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-muted)' }}>VS</div>
                          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>Win Probability</div>
                        </div>
                        <div style={{ textAlign: 'center', flex: 1, minWidth: '120px', padding: '1rem', borderRadius: '12px',
                          background: !isFavoured ? 'rgba(239,68,68,0.06)' : 'rgba(255,255,255,0.02)',
                          border: !isFavoured ? '1px solid rgba(239,68,68,0.2)' : 'none' }}>
                          <div style={{ fontSize: '2.2rem', marginBottom: '0.4rem' }}>🏏</div>
                          <div style={{ fontSize: '0.88rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>{team2}</div>
                          <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: '3rem', fontWeight: 800, lineHeight: 1, marginBottom: '0.5rem', color: !isFavoured ? '#ef4444' : '#94a3b8' }}>{oppProb}%</div>
                          {!isFavoured && <span style={{ background: 'rgba(239,68,68,0.15)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '20px', padding: '0.2rem 0.7rem', fontSize: '0.7rem', fontWeight: 700 }}>FAVOURED ✓</span>}
                        </div>
                      </div>
                      ...
                      <div style={{ marginBottom: '1.25rem' }}>
                        {[
                          { label: '🇱🇰 Sri Lanka', pct: slProb,  fav: isFavoured,  yc: '#10b981' },
                          { label: `🏏 ${team2}`,   pct: oppProb, fav: !isFavoured, yc: '#ef4444' },
                        ].map((bar, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.6rem' }}>
                            <span style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', minWidth: '110px' }}>{bar.label}</span>
                            <div style={{ flex: 1, height: '10px', background: 'rgba(255,255,255,0.07)', borderRadius: '5px', overflow: 'hidden' }}>
                              <div className="pb-fill" style={{ height: '100%', borderRadius: '5px', width: `${bar.pct}%`,
                                background: bar.fav ? `linear-gradient(90deg,${bar.yc},${bar.yc}aa)` : 'linear-gradient(90deg,#64748b,#94a3b8)' }} />
                            </div>
                            <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text-primary)', minWidth: '40px', textAlign: 'right' }}>{bar.pct}%</span>
                          </div>
                        ))}
                      </div>

                      {/* Recommendation */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', borderRadius: '10px', padding: '1rem 1.25rem', marginBottom: '1.25rem',
                        background: isFavoured ? 'linear-gradient(135deg,rgba(16,185,129,0.12),rgba(16,185,129,0.04))' : 'linear-gradient(135deg,rgba(239,68,68,0.12),rgba(239,68,68,0.04))',
                        border: `1px solid ${isFavoured ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}` }}>
                        <span style={{ fontSize: '1.3rem' }}>{isFavoured ? '✅' : '⚠️'}</span>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: '1rem', color: isFavoured ? '#10b981' : '#ef4444' }}>
                          ...
                          {(prediction.key_factors || []).map((factor, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', padding: '0.6rem 0.85rem', border: '1px solid rgba(255,255,255,0.05)' }}>
                              <span style={{ fontSize: '0.95rem', flexShrink: 0 }}>
                                {factor.toLowerCase().includes('form') ? '📈' : factor.toLowerCase().includes('dew') ? '💧' : factor.toLowerCase().includes('toss') ? '🪙' : factor.toLowerCase().includes('head') ? '⚔️' : '📊'}
                              </span>
              ...
              {strategies.length > 0 && (
                <div style={{ marginBottom: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                    <div>
                      <h2 style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: '1.4rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.3rem' }}>
                        {isFavoured ? '🏆 Match Winning Plan' : '🔄 Match Turnaround Strategies'}
                      </h2>
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.83rem', maxWidth: '600px', lineHeight: 1.5 }}>
                        {isFavoured
                    ...
                    <span style={{ background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.3)', color: '#c084fc', borderRadius: '20px', padding: '0.3rem 0.9rem', fontSize: '0.75rem', fontWeight: 600 }}>
                    ...
                    {strategies.map((card, idx) => {
                      ...
                      return (
                        <div key={idx} className="strat-card"
                          style={{ borderRadius: '14px', padding: '1.4rem', background: col.bg, border: `1px solid ${col.border}`, animationDelay: `${idx * 0.08}s` }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                            <div style={{ width: '4px', height: '28px', borderRadius: '2px', background: col.accent, flexShrink: 0 }} />
                            <span style={{ fontSize: '1.3rem' }}>{card.icon}</span>
                            <h3 style={{ fontSize: '0.95rem', fontWeight: 700, lineHeight: 1.3, flex: 1, color: col.accent, margin: 0 }}>{card.title}</h3>
                          </div>
                          <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 1rem', display: 'flex', flexDirection: 'column', gap: '0.55rem' }}>
                            {card.bullets.map((b, i) => (
                              <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', fontSize: '0.84rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                                <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: col.accent, flexShrink: 0, marginTop: '0.45rem' }} />
                                <span>{b}</span>
                              </li>
                            ))}
                          </ul>
                          <div style={{ borderTop: `1px solid ${col.border}`, paddingTop: '0.85rem', fontSize: '0.8rem', fontStyle: 'italic', lineHeight: 1.5, color: col.accent, opacity: 0.9 }}>
                            {card.motivation}
                          </div>
                        </div>
                      )
