...
function VenueSearch({ venues, venueId, onSelect }) {
  ...
  const filtered = venues
    .filter((v) => v.display_name.toLowerCase().includes(query.toLowerCase()))
    .slice(0, 12);
  ...
  const handleKey = (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      setQuery(filtered[highlighted].display_name);
      setOpen(false);
          ...
          {filtered.map((v, i) => (
            <div
              ...
              style={{
                ...
                fontSize: "0.9rem",
                background:
                  i === highlighted ? "rgba(245,158,11,0.15)" : "transparent",
                color: i === highlighted ? "#f59e0b" : "#cbd5e1",
                borderBottom:
                  i < filtered.length - 1 ? "1px solid #334155" : "none",
              }}
...
export default function VenueWeather() {
  ...
  const handleVenueSelect = (v) => {
    ...
    setVenueId(String(v.venue_id)); // ← use venue_id not id
    setVenueName(v.display_name); // ← use display_name not name
    setResults(null);
  };
  ...
  const analyse = async () => {
    ...
    try {
      const [stats, par, phase, weather] = await Promise.all([
        ...
        matchDate
          ? getWeatherConditions(venueName, slRole === "batting", matchDate)
          : null,
      ]);
      setResults({
        stats: stats.data,
  ...
  const riskWidth = (score) =>
    `${Math.min(100, Math.round((score || 0) * 100))}%`;

  return (
    <div className="page fade-in">
      <h1 className="page-title">🏟️ Venue & Weather Analysis</h1>
      <p className="page-subtitle">
        Opponent-adjusted par scores, phase analytics and weather conditions
      </p>
        ...
        <div className="grid-2" style={{ gap: "1rem" }}>
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            <select
              className="form-control"
              value={slRole}
              onChange={(e) => setSlRole(e.target.value)}
            >
            ...
            <div className="grid-4" style={{ marginBottom: "1rem" }}>
              <div className="stat-card">
                ...
                <div className="stat-sub">
                  {results.par?.recommended?.source}
                </div>
              </div>
              <div className="stat-card">
                <div className="stat-value">
                  {results.par?.overall_venue_average?.par_score || "—"}
                </div>
                <div className="stat-label">Overall Venue Avg</div>
                <div className="stat-sub">
                  {results.par?.overall_venue_average?.matches} matches
                </div>
              </div>
              <div className="stat-card">
                <div className="stat-value">
                  {results.par?.sl_at_venue?.par_score || "—"}
                </div>
                <div className="stat-label">SL at Venue</div>
                <div className="stat-sub">
                  {results.par?.sl_at_venue?.matches} matches
                </div>
              </div>
              <div className="stat-card">
                <div className="stat-value">
                ...
                <div className="stat-sub">
                  {results.par?.sl_vs_opponent_at_venue?.matches || 0} matches
                </div>
              </div>
            </div>
              ...
              <div className="card">
                ...
                <div className="grid-2">
                  {[
                    ...
                    {
                      v: results.stats.avg_first_innings_score,
                      l: "1st Innings Avg",
                    },
                    {
                      v: results.stats.avg_second_innings_score,
                      l: "2nd Innings Avg",
                    },
                    {
                      v: results.stats.toss_recommendation,
                      l: "Toss Recommendation",
                    },
                  ].map((s) => (
                    <div key={s.l} className="stat-card">
                      <div
                        ...
                        style={{
                          fontSize:
                            typeof s.v === "string" ? "0.9rem" : "1.8rem",
                  ...
                  {[
                    {
                      ...
                      pct: results.stats.bat_first_win_pct,
                      wins: results.stats.bat_first_wins,
                    },
                    {
                      label: "Chase Wins",
                      pct: results.stats.chase_win_pct,
                      wins: results.stats.chase_wins,
                    },
                  ].map((r) => (
                ...
                <tbody>
                  {results.phase.map((p) => (
                    <tr key={p.phase}>
                      ...
                      <td>
                        <span
                          className={`badge ${p.avg_runs > 8 ? "badge-high" : p.avg_runs > 6 ? "badge-medium" : "badge-low"}`}
                        >
                          {p.avg_runs > 8
                            ? "High Scoring"
                            : p.avg_runs > 6
            ...
            >
              ...
              <div style={{ marginBottom: "0.75rem" }}>
                <span className="badge badge-blue">
                  {results.weather.weather_source}
                </span>
              </div>
              <div className="grid-3" style={{ marginBottom: "1.5rem" }}>
                {[
                  {
                    label: "Humidity",
                    value: `${results.weather.conditions?.humidity_pct}%`,
                    icon: "💧",
                  },
                  {
                    label: "Rain Probability",
                    value: `${results.weather.conditions?.rain_probability_pct}%`,
                    icon: "🌧️",
                  },
                  {
                    label: "Cloud Cover",
                    value: `${results.weather.conditions?.cloud_cover_pct}%`,
                    icon: "☁️",
                  },
                ].map((c) => (
              ...
              <div className="grid-3" style={{ marginBottom: "1.5rem" }}>
                {[
                  {
                    label: "Dew Risk",
                    score: results.weather.risk_scores?.dew_risk,
                    badge: results.weather.risk_scores?.dew_risk_label,
                  },
                  {
                    label: "Rain Risk",
                    score: results.weather.risk_scores?.rain_risk,
                    badge: results.weather.risk_scores?.rain_risk_label,
                  },
                  {
                    label: "Swing Conditions",
                    score: results.weather.risk_scores?.swing_conditions,
                    badge: results.weather.risk_scores?.swing_label,
                  },
                ].map((r) => (
                  <div
                    key={r.label}
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        className="badge"
                        style={{
                          background: `${riskColor(r.badge)}22`,
                          color: riskColor(r.badge),
                          border: `1px solid ${riskColor(r.badge)}44`,
                        }}
                      >
                        {r.badge}
                      </span>
                    ...
                    <div className="risk-bar">
                      <div
                        className="risk-fill"
                        style={{
                          width: riskWidth(r.score),
                          background: riskColor(r.badge),
                        }}
                      />
                    </div>
                  </div>
              ...
              >
                ...
                <span
                  className={`badge ${results.weather.recommendation_strength === "STRONG" ? "badge-high" : "badge-medium"}`}
                >
