...
export const getVenueStats = (id, opponent) =>
  api.get(`/venues/${id}/stats`, { params: opponent ? { opponent_team: opponent } : {} })
export const getVenueParScore = (id, role, opponent) =>
  api.get(`/venues/${id}/par-score`, { params: { sl_role: role, opponent_team: opponent } })
...
export const getPlayerBatting = (name) => api.get(`/players/${encodeURIComponent(name)}/batting`)
export const getPlayerBowling = (name) => api.get(`/players/${encodeURIComponent(name)}/bowling`)
...
export const analyzeOpponentXI = (data) => api.post('/xi/analyze-opponent', data)
...
export const TEAMS = [
  ...
  'Ireland', 'Netherlands', 'Scotland', 'Nepal', 'Oman',
]
...
export const SL_SQUAD = [
  ...
  'M Pathirana', 'A Dananjaya', 'J Liyanage', 'B Fernando',
  'MADI Hemantha', 'RMVD Gunaratne',
]
